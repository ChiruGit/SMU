---
title: "Unit 6 Case Study - Real Time Location Systems"
output: html_document
---

# Introduction
In this case study, we attempt to determine indoor location by analyzing the signal strength of several wireless access points as measured by a scanning device.  The data consists of an "offline" training set and an "online" test set.  The offline data contains measurements of signal strength from a grid of specified measurement points that are one meter apart from each other.  There are 6 wireless access points and the signal for each access point is measured from each of these points at 8 different rotational angles that are 45 degrees apart.  The online data contains similar measurements for which we will attempt to predict the location.

The chosen method of analysis for this case study is k-Nearest Neighbors (or kNN).  kNN works by comparing a given point with a predetermined number (k) of closest points to it.  These neighboring points "vote" to determine the value of the point being analyzed.  After the initial kNN analysis is performed, we will also examine possible enhancements including weighting methods.  

Before the kNN analysis can be performed, a significant amount of cleaning and data exploration will need to be done.  The data will need to be reformatted into a shape that is conducive to kNN.  Any anomalies that are detected in the data (such as missing values) will also need to be dealt with.  

The data sets and much of the code in this case study are based on Chapter 1 of "Data Science in R: A Case Studies Approach to Computational Reasoning and Problem Solving" by Noland & Lang.  

# Methods and Results
## Data Cleaning and Exploration
Before we are able to analyze this signal data using k-Nearest Neighbors, we must first clean and explore the data.  We will start byreading the data file into a variable named txt.  
```{r}
txt = readLines("Data/offline.final.trace.txt")
```

Next, we tally number of lines in file that begin with a '#' character  
```{r}
sum(substr(txt, 1, 1) == "#")
```
...
Next, we need to convert the data into a structure that is easier to analyze.  The original structure contains a non-uniform number of readings for each line.  We will reshape the data to have one measurement per line.  
...
## Analysis of Anomalous MAC Addresses
...
onlineRedo = readData("Data/online.final.trace.txt", subMacs = macs)
...
onlineRedo2 = readData("Data/online.final.trace.txt", subMacs = macs)
...
## Weighted k-Nearest Neighbors Analysis
...
We can now rerun kNN using the weighted data.
```{r}
trainSS = reshapeSS(offlineSubsetRedo, varSignal = "avgSignal")
...
Unfortunately I have run out of time and have not yet been able to get the weighting portion of this assignment working properly.  I have spent about 30 hours on this project over the past 2 weeks, but was not quite able to make it to the finish line.  However, if I was able to calculate the weights, the next step would be to determine the optimal number of nearest neighbors and then run kNN and compare the error rate to the non-weighted error rate (as we did with the previous versions of the data).  We would expect t0 see a lower error rate for the weighted data.  
